...
HTTP_USER_AGENT = "grain-engine/4.1 (Ukraine grain procurement)"
...
def logistics_range(distance_km: float) -> Tuple[float, float]:
    ...
    if distance_km >= LOGISTICS[-1][1]:
        low, high, cost_low, cost_high = LOGISTICS[-1]
        extra = distance_km - high
        # После 900 км продолжаем последнюю ставку линейно по тем же 100 км.
        step = max(1.0, cost_high - cost_low)
        blocks = extra / 100.0
        return float(cost_low + step * blocks), float(cost_high + step * blocks)
...
def geocode_location(origin_text: str, cache_path: Path = DEFAULT_LOCATION_CACHE) -> Tuple[float, float]:
    """Ищет населённый пункт Украины через Nominatim и сохраняет результат в кэш."""
    cached = get_cached_location(origin_text, cache_path)
    if cached:
        return float(cached["latitude"]), float(cached["longitude"])

    query = normalize_location_name(origin_text)
    if "украин" not in query.lower() and "украї" not in query.lower():
        query = f"{query}, Украина"

    data = _http_get_json(
        GEOCODER_URL,
        {"q": query, "format": "jsonv2", "limit": 1, "countrycodes": "ua", "accept-language": "uk,ru"},
    )
    if not data:
        raise ValueError(f"Не удалось геокодировать локацию: {origin_text}")

    item = data[0]
    latitude = float(item["lat"])
    longitude = float(item["lon"])
    cache_location(origin_text, latitude, longitude, item.get("display_name", ""), cache_path)
    return latitude, longitude


def resolve_location(origin_text: str, cache_path: Path = DEFAULT_LOCATION_CACHE) -> Tuple[float, float]:
    """Локальная база -> кэш -> автоматическое геокодирование."""
    ...
    return geocode_location(key, cache_path)


def route_distance_km(a: Tuple[float, float], b: Tuple[float, float]) -> float:
    """Возвращает дорожное расстояние по OSRM в километрах."""
    lon1, lat1 = a[1], a[0]
    lon2, lat2 = b[1], b[0]
    coordinates = f"{lon1},{lat1};{lon2},{lat2}"
    data = _http_get_json(
        f"{ROUTER_URL}/{coordinates}",
        {"overview": "false", "steps": "false"},
    )
    if data.get("code") != "Ok" or not data.get("routes"):
        raise ValueError(f"OSRM не нашёл маршрут: {data.get('code', 'unknown')}")
...
def distance_to_destination(
    ...
    try:
        road = route_distance_km(origin_xy, destination_xy)
        return round(road, 1), "дорожный маршрут OSRM"
    except Exception:
        # Если внешний роутер временно недоступен, не ломаем закупочный расчёт.
        straight = haversine_km(origin_xy, destination_xy)
        road = straight * ROAD_FACTOR
        return round(road, 1), "резервная оценка по координатам"
